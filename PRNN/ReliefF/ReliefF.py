...
from ReliefF import Base


class ReliefF(Base.Base):
    def __init__(self, max_iter, rank, neighbors):
        """
            ...
            for j in range(n):
                ...
                for label in label_index.keys():
                    if label == sample_y:
                        ...
                        near_miss_sum += pre_near_miss_sum * label_probability[label] / (1 - label_probability[sample_y])

                self._weight[j] += (near_miss_sum - near_hit_sum) / self.neighbors

        f_index = np.argsort(-self._weight)
        t = 0
        for i in f_index:
            if(t<self.rank):
                self._important_weight[i] = self._weight[i]
            else:
                break
            t = t + 1
