...
def delete_redundancy_feature_inPFront(Obj_max_DF, Obj_class):
    geneName = []
    for i in range(5):
        flag = False
        max_geneName = ""
        max_score = 0
        for index, cls in enumerate(Obj_class):
            if(i == cls):
                if(flag == False):
                    max_score = Obj_max_DF.iloc[index, 0]
                    max_geneName = Obj_max_DF.index.tolist()[index]
                    flag = True
                else:
                    if(Obj_max_DF.iloc[index, 0] > max_score):
                        max_score = Obj_max_DF.iloc[index, 0]
                        max_geneName = Obj_max_DF.index.tolist()[index]
        if(flag):
            geneName.extend([max_geneName])
    return geneName

def NN_FS(data_x,data_y,batch_size,lr, geneLen):
    class_num = data_y.nunique()
    start_dim = data_x.shape[1]
    end_dim = class_num
    mean_split_space = math.pow(2, (math.log2(start_dim / end_dim)) / 5)
    ...
    Obj_max_DF.columns = ['score']
    Obj_max_DF = Obj_max_DF.sort_values(by='score', axis=0, ascending=False)

    # e = int(len(geneName)*p)
    e = geneLen
    final_geneName = Obj_max_DF.iloc[0:e, :].index.tolist()
